...
- Interface em **português brasileiro** (idioma nativo); inglês via menu **Idioma** (e KI18n)
...
O executável fica em `<diretório-de-build>/bin/webappstation`. Se a pasta `build/` estiver ocupada ou com permissão errada (ex.: build anterior via Docker), use outro diretório, por exemplo `build-agent`:

```bash
cmake -B build-agent -G Ninja -DCMAKE_BUILD_TYPE=Release
cmake --build build-agent
./build-agent/bin/webappstation
```
...
(ou o mesmo com `-B build-agent` / `--test-dir build-agent`)
